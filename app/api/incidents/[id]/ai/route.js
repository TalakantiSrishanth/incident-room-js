...
export async function POST(req, { params }) {
  ...
  try {
    ...
    const updatesText = updates.map(u => `[${u.author_name}]: ${u.message}`).join('\n') || 'No updates yet.';
    ...
    try {
      ...
      const response = await model.generateContent(prompts[type]);
    ...
    const aiResult = await AIResult.create({ incident_id: id, type, result_text });
...
export async function GET(_req, { params }) {
  ...
  try {
    await connectDB();
    const results = await AIResult.find({ incident_id: id }).sort({ created_at: -1 }).limit(10);
